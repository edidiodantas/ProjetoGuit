import { describeFuryuWebviewError } from "core/config/furyu/errors";
    ...
    const listener = webView.onDidReceiveMessage(async (msg) => {
      ...
      for (const handler of handlers) {
        try {
          ...
          const furyuError = describeFuryuWebviewError(e);
          if (furyuError) {
            e.message = furyuError.message;
          }
          ...
          if (furyuError) {
            message = furyuError.message;
            e.message = furyuError.message;
            const buttons = furyuError.switchToOnline
              ? [furyuError.switchLabel, "Show Logs"]
              : ["Show Logs", "Troubleshooting"];
            vscode.window
              .showErrorMessage(message, ...buttons)
              .then((selection) => {
                if (selection === furyuError.switchLabel && furyuError.switchToOnline) {
                  void this.request("furyuSwitchToOnline", undefined);
