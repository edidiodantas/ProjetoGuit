...
import { maybeLogStreamChatTelemetry } from "../config/furyu/telemetryGate.js";
...
${prompt}`;
  ...
  async *streamChat(
    ...
    // Opt out with allowAnonymousTelemetry: false or sendAnonymousTelemetry: false.
    // The local profile sets both and Telemetry.setup(false) before this runs.
    try {
      let provider = this.providerName;
      const telemetryOptions = { ...completionOptions, provider };
      await maybeLogStreamChatTelemetry(
        Telemetry.allow,
        (event, options) => anonymousTelemetryLog(event, options),
        telemetryOptions,
      );
    } catch (error) {
