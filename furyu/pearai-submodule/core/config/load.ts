...
async function addDefaults(
  ...
  options?: { skipModels?: boolean },
) {
  // The local profile must not call Pear's getDefaultConfig.
...
async function loadFullConfigNode(
  ...
  if (furyuProfile === "local") {
    await addDefaults(serialized, { skipModels: true });
    const embeddingModelAvailable = await ollamaHasEmbeddingModel();
    serialized = applyFuryuLocalMode(serialized, { embeddingModelAvailable });
  } else {
