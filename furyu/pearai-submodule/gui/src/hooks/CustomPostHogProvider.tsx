import { shouldInitializePostHog } from "core/config/furyu/telemetryGate";
...
const CustomPostHogProvider = ({ children }: PropsWithChildren) => {
  ...
  useEffect(() => {
    // allowAnonymousTelemetry used to be ignored here: init and opt-in always ran.
    if (!shouldInitializePostHog(allowAnonymousTelemetry)) {
      if (client) {
